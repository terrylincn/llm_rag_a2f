...
PLAYERSTREMING = "/World/LazyGraph/PlayerStreaming"
URL = "localhost:50051"
SAMPLE_RATE = 16000

def push_audio_track(url=URL, audio_data=None, samplerate=SAMPLE_RATE, instance_name=PLAYERSTREMING):
    """
...
def push_audio_track_stream(url, audio_data, samplerate, instance_name):
    ...
    sleep_between_chunks = 0.08  # ADJUST
    block_until_playback_is_finished = True  # ADJUST

    with grpc.insecure_channel(url) as channel:
        #if not channel._connectivity_state.connectivity:
        #    return
...
def main():
    ...
    if len(sys.argv) < 3:
        print("Format: python test_client.py PATH_TO_WAV INSTANCE_NAME SERVER_URL")
        return
